...
def insert_user_info(name, age, email):
    conn = sqlite3.connect('payhealth.db')
    cursor = conn.cursor()
    cursor.execute("INSERT INTO user_info (name, age, email) VALUES (?, ?, ?)", (name, age, email))
    user_id = cursor.lastrowid
    conn.commit()
    conn.close()
    return user_id

# Function to insert error info into the database
def insert_error_info(user_id, field_name, error_type):
    conn = sqlite3.connect('payhealth.db')
    cursor = conn.cursor()
    cursor.execute("INSERT INTO error_info (user_id, field_name, error_type) VALUES (?, ?, ?)", (user_id, field_name, error_type))
    conn.commit()
    conn.close()
...
def payhealth():
    ...
    user_id = insert_user_info(name, age, email)

    # Insert error info into the database if there are errors
    for result in validation_results:
        if not result['success']:
            insert_error_info(user_id, result['field'], result['error'])
